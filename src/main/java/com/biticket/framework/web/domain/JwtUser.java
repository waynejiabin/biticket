...
public class JwtUser implements UserDetails {
    ...
    /**写一个能直接使用Member创建jwtUser的构造器*/
    public JwtUser(Member member) {
        id =String.valueOf( member.getMemberId());
        username = member.getLoginName();
        password = member.getMemberPassword();
        userType = member.getMemberType();
        userStatus = member.getMemberStatus();
        authorities = Collections.singleton(new SimpleGrantedAuthority(String.valueOf(member.getMemberType())));
