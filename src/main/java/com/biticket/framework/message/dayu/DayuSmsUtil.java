...
import com.biticket.common.thirdUtil.JsonUtil;
...
public class DayuSmsUtil {
    ...
    private static final Logger log = LoggerFactory.getLogger(DayuSmsUtil.class);
    /**
    ...
    public static void init(DayuSmsConfig config) {
        DayuSmsUtil.config = config;
        DayuSmsUtil.isOpen = config.getIsOpen();
        DayuSmsUtil.signature = config.getSignature();
     ...
     * @return
     */
    public static boolean send(Map<?, ?> param, String templateCode, String[] recNum) {
        return send(param, templateCode, "", recNum);
    }
     ...
     * @return
     */
    public static boolean send(Map<?, ?> param, String templateCode, String extendStr,
                               String[] recNum) {
        return send(param, templateCode, DayuSmsConstants.SMS_NORMAL, "", recNum);
    }
     ...
     * @return
     */
    public static boolean send(Map<?, ?> param, String templateCode, String type, String extendStr,
                               String[] recNum) {
        return send(JsonUtil.object2String(param), templateCode, type, extendStr, recNum);
    }
     ...
     * @return
     */
    public static boolean send(String param, String templateCode, String type, String extendStr,
                               String[] recNum) {
        ...
        return send(param, resNums.substring(0, resNums.length() - 1), templateCode, type, extendStr);
    }
    ...
    private static boolean send(String param, String recNum, String templateCode, String type, String extendStr) {
        ...
        SendSmsRequest request = packRequest(param, recNum, templateCode, type, extendStr);
...
*/
        if (response.getCode() != null && response.getCode().equals("OK")) {
            log.info("返回信息：【" + response.getCode() + "】" + response.getMessage());
            return true;
        }
       return  false;
